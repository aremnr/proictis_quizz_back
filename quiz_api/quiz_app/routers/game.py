...
from quiz_app.schemas import AdminValidate, Admin, Question
...
async def create_game(quiz_id: str, game_owner: str = "1"):
    game_id = 'a'
    games[game_id] = Game(game_id = game_id, game_owner=game_owner, quiz_id=quiz_id)
...
async def game(websocket: WebSocket, game_id: str, db: Session = Depends(get_db)):
    ...
    try:
        while True:
            ...
            if json_data['headers']['type'] == "game" and json_data['headers']['JWT'] == '1':
                game.question = crud.get_question(db=db, quiz_id=game.quiz_id, pcl=game.pcl)
                await game.manager.JSON_broadcast(json.loads(game.question.json()))
            ...
            if json_data['headers']['type'] == "check_answer" and game.is_started:
                ...
                print(3)
            if json_data['headers']['type'] == "get_answer" and json_data['headers']['JWT'] == '1' and game.is_started:
            ...
            if json_data['headers']['type'] == "end_game" and json_data['headers']['JWT'] == '1' and game.is_started:
