...
@router.post("/quiz/{question_id}/add")
def add_answer(question_id: str, model: schemas.Answer, db: Session = Depends(get_db)):
    return crud.add_answer(db=db, answer=model, question_id=question_id)


@router.get("/quiz/{quiz_id}", response_model=schemas.Quiz)
def get_quiz_by_id(quiz_id: str, db: Session = Depends(get_db)):
    return crud.get_quiz(db=db, quiz_id=quiz_id)


@router.get("/quiz/{quiz_id}/question", response_model=schemas.Question)
