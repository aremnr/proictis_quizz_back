...
@router.post("/quiz/add", tags=["quiz"], status_code=201)
...
@router.post("/quiz/{quiz_id}/add/question", tags=["quiz"], status_code=201)
...
@router.post("/quiz/{quiz_id}/{pcl}/add", tags=["quiz"], status_code=201)
...
@router.get("/quiz/{quiz_id}", response_model=schemas.Quiz, tags=["quiz"], status_code=200)
def get_quiz_by_id(quiz_id: str, db: Session = Depends(get_db)):
    return crud.get_quiz(db=db, quiz_id=quiz_id)


@router.get("/quiz/{quiz_id}/question", response_model=schemas.Question, tags=["quiz"], status_code=200)
...
@router.get("/quiz/{quiz_id}/ans_check", response_model=schemas.Check, tags=["quiz"], status_code=200)
...
@router.get("/get_quizzes", status_code=200)
